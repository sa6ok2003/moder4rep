...
import sqlite3
from .sqlit import info_members, reg_one_channel, reg_channels,del_one_channel,cheak_traf,obnovatrafika,reg_admin,list_adminov,obnovatrafika2,cheak_traf2,cheak_person,obnova_pers,info_chyornaya_vdova,info_filmyshd,info_films_online_everyday
...
async def admin_ka(message: types.Message):
    ...
    if id in ADMIN_ID:
        ...
        fain_but = types.InlineKeyboardButton(text='Fain', callback_data='filmyshd')
        yulya_but = types.InlineKeyboardButton(text='Юля', callback_data='films_online_everyday')

        markup.add(bat_a,bat_e,bat_j)
        markup.add(den_but,fain_but,yulya_but) #Инфо о партнерах
...
async def cheak_traaf(message: types.Message):
    ...
    if message.chat.id == PARTNERS3: #FAIN
        a = info_filmyshd()  # Вызов функции из файла sqlit
        await bot.send_message(message.chat.id, f'Счетчик подписок: {a}\n'
                                                     f'Канал : @filmyshd')
...
@dp.callback_query_handler(text='filmyshd')  # ТРАФИК У FAINA
async def filmyshd(call: types.callback_query):
    a = info_filmyshd() # Вызов функции из файла sqlit
    await bot.send_message(call.message.chat.id, f'Счетчик подписок: {a}\n'
                                                 f'Канал : @filmyshd')
